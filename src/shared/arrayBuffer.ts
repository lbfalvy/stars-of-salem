 ...
 */
export function copyArrayBuffer(from: ArrayBuffer, start: number, length: number, to: ArrayBuffer,
                                offset: number): void {
...
export function bufferArrayToArrayBuffer(buf: Uint8Array[]): ArrayBuffer {
    ...
    buf.reduce((offset, next) => {
        copyArrayBuffer(next.buffer, next.byteOffset, next.byteLength, result, offset);
