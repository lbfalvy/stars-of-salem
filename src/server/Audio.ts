...
function worker_function():void {
    ...
    const handleBuffer = (client:Client, samples:Float32Array):void => {
        if (samples.length != sample_count) {
            throw new Error('bad sample count');
        }
    ...
    const processCommand = (client:Client, cmd:any):void => {
        ...
        if (cmd.leaveSpaces instanceof Array) {
            ...
            for (const space_name of leaveSpaces) {
                ...
                if (space.clients.size == 0) {
                    state.spacesByName.delete(space_name);
                }
            }
        }
        ...
        if (typeof cmd.setVolume == 'number') {
            client.volume = cmd.setVolume;
        }
    };
    ...
    worker_threads.parentPort.on('message', (port_or_initconf:any) => {
        if (port_or_initconf instanceof worker_threads.MessagePort) {
            if (!sample_count) {
                throw new Error('Specify the sample count before registering clients');
            }
            const client:Client = {
                spaces: new Set(),
            ...
            client.port.on('message', samples_or_command => {
                if (samples_or_command instanceof Float32Array) {
                    ...
                    processCommand(client, samples_or_command);
                }
            });
            return;
        }
...
export default class Audio {
    ...
    public constructor() {
        if (!Audio.sampleCount || !Audio.sampleRate) {
            throw new Error('Set the sample properties before registering a client');
        }
    ...
    public input(samples: Float32Array):void {
        if (samples.length != Audio.sampleCount) {
            throw new RangeError('Bad sample count!');
        }
